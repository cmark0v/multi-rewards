...
echo "this script changes the solc version in brownie config and all contacts and libraries in contracts folder"
fi
export CHK=`echo $1|grep -ow '[0-9]\\.[0-9]\\.[0-9]\{1,2\}'`
if [ $CHK ] 
then
    sed -i "s/pragma solidity \\^$VERSION/pragma solidity \\^$CHK/" brownie-config.yaml
    echo "change brownie"
